...
var _ sbased2.StringValuesProvider = &EntriesProvider{}
...
func extractStringValues(env []string, prefix string) (
	sbased2.StringValues, error,
) {
	var ambiguousKeys []string

	stringValues := make(sbased2.StringValues, 0, len(env))
		...
		if len(groups) == rePrefixed.NumSubexp()+1 {
			if reSubKey.MatchString(groups[1]) {
				stringValues = append(
					stringValues,
					&sbased2.StringValue{
						Key: strings.ToLower(groups[1][1:]),
...
func (e *EntriesProvider) GetStringValues() sbased2.StringValues {
